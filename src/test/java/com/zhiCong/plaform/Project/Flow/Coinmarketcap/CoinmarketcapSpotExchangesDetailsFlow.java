...
public class CoinmarketcapSpotExchangesDetailsFlow extends BaseFlow {
  ...
  public boolean isBinanceExchangeProportionPrecise() {
    ...
    BigDecimal totalBalance =
        new BigDecimal(
            keepNumbersDecimalPoints(
                waitForElement(coinmarketcapSpotExchangesDetailsPage.spotExchangesDetailsBalance)
                    .getText()));
    System.out.println("Binance total balance : " + totalBalance);
    List<WebElement> filerCurrency =
        coinmarketcapSpotExchangesDetailsPage.chartFilterCurrencyButtonList;
    for (int i = 0; i < filerCurrency.size() - 1; i++) {
      ...
      if (checkForElement(
          coinmarketcapSpotExchangesDetailsPage.acceptCookiesAndContinueButton, 5)) {
      ...
      BigDecimal expected =
          new BigDecimal(
              keepNumbersDecimalPoints(
                  coinmarketcapSpotExchangesDetailsPage
                      .chartFilterProportionList
                      .get(i)
                      .getText()));
      ...
      // 获取该货币的金额
      List<WebElement> currencyAmountList =
          coinmarketcapSpotExchangesDetailsPage.refinedBalanceList;
