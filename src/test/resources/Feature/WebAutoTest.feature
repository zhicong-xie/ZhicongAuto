Feature: Web auto testing
  ...
  Scenario: check Binance Exchange Proportion
    When the user click Markets button on LOOM Coinmarketcap currency details screen top banner
    And the user select "100" rows on LOOM Coinmarketcap currency details screen rows dropDown
    Then the user able to see "Binance" volume percentage is correct on LOOM Coinmarketcap currency details screen

  @AC7.8 @AC7.9 @AC7.10
  Scenario: select MetaMask on Supported wallets
    Given the user able to see LOOM Coinmarketcap currency details screen
    When the user select Metamask icon on LOOM Coinmarketcap currency details screen support wallets view
